...
## Architecture
the lab uses standard Wazuh single node deployment
...
I chose to use Wazuh as my platform because it is very commonly used in many SOC environments and supports multiple different operating systems. If I were to expand upon this home lab and continue it the next step I would take would be to add more agents like a Windows vm with the goal of getting an even more diverse set of data. I would also explore adding things like custom detection rules in order to simulate more specific attack scenarios. I would also enable the vulnerability detection module to scan endpoints for known CVEs. Automating wazuh to be able to block certain ips as well when certain alert types fire could also be something I would explore. This project overall gave me great overall hands on experience with simulating a real workflow in a SOC environment.
